...
from Gojo.utils.champu_ban import admin_filter
...
import asyncio
import time
from Gojo.utils.database import add_served_chat
from pyrogram.types import Message, InlineKeyboardMarkup, InlineKeyboardButton
from Gojo.utils.database import get_assistant
import asyncio
from Gojo.misc import SUDOERS
from Gojo.mongo.afkdb import PROCESS
from pyrogram import Client, filters
...
from Gojo.utils.database import get_assistant, is_active_chat
...
def welcomepic(pic, user, chatname, id, uname, brightness_factor=1.3):
    background = Image.open("Gojo/assets/wel2.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp, brightness_factor=brightness_factor) 
    pfp = pfp.resize((635, 635))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('Gojo/assets/font.ttf', size=70)
    welcome_font = ImageFont.truetype('Gojo/assets/font.ttf', size=61)
...
async def add_all(client, message):
    ...
    try:
        ...
        async for dialog in userbot.get_dialogs():
            if dialog.chat.id == -1002111155780:
                continue
            try:
